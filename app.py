...
clf = pickle.load(open(r'ml/nlp_model.pkl', 'rb'))
vectorizer = pickle.load(open(r'ml/tranform.pkl', 'rb'))


def create_similarity():
    data = pd.read_csv(r'processed_data/main_data.csv')
...
def rcmd(m):
    ...
    if m not in data['movie_title'].unique():
        ...
        for i in range(len(lst)):
            a = lst[i][0]
            l_titles.append(data['movie_title'][a])
        return l_titles
...
def recommend():
    ...
    # don't using function above because in this list digits been mapped
